...
var InteractionState = class InteractionState {
	...
	* is still blank.
	*/
	sessionPresetSwitchedIds = /* @__PURE__ */ new Set();
...
*/
function mergeCommandResults(entries, commandResults) {
...
function commandResultToV2(entry) {
	...
	const created = typeof info.time === "object" && info.time !== null && "created" in info.time ? Number(info.time.created) : 0;
...
function registerSessionV2Routes(register) {
	...
	register("GET", "/api/session/:sessionID/message", "json", async (req, ctx) => {
		...
		if (commandResults.length > 0) {
			...
			remapped.sort((a, b) => {
				return (a.time?.created ?? 0) - (b.time?.created ?? 0);
			});
		}
		return json(200, {
				...
				const consumeHost = async (stream) => {
					for await (const frame of stream) {
						...
						if (payload.type === "host/session-added" || payload.type === "host/session-removed") {
							ctx.state.invalidateSession();
							scheduleListRefresh();
							if (payload.type === "host/session-added") for (const event of hostSessionAddedEvents(ctx, payload)) sendToClient(event);
						}
					}
				};
...
//# sourceMappingURL=http-D4dw7U8G.js.map