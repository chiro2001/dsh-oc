...
* order puts e.g. deepseek-v4-flash first even when the operator configured
* deepseek-v4-pro as the default. Read the resolved settings value and map the
* provider back to its opencode-facing id.
*/
async function configuredDefaultModel(ctx) {
	try {
		const value = ((await rpc(ctx, "settings.describe", {})).namespaces?.find((entry) => entry.ns === "agent-default-model"))?.value;
		...
		return {
			...
			modelID: value.model,
			...reasoningEffort === void 0 || reasoningEffort === "default" ? {} : { variant: reasoningEffort }
		};
	} catch (error) {
...
async function reconcileModelSelection(ctx, sessionId) {
	...
	const variantMatches = cached.variant === void 0 || currentVariant === cached.variant;
	if (providerMatches && modelMatches && variantMatches) return;
	try {
		await rpc(ctx, "session.selectModel", {
			sessionId: sid(sessionId),
			provider: dshProviderId(cached.providerID),
			model: cached.modelID,
			...cached.variant === void 0 ? {} : { reasoningEffort: cached.variant }
		});
...
//# sourceMappingURL=http-CVe6MU9J.js.map