...
function subagentMetadataForHistory(ctx, parentSessionId, call) {
	...
	const child = ctx.state.subagentChildForCall(parentSessionId, call.callId) ?? (() => {
		...
		return [...ctx.state.subagentChildren.values()].filter((candidate) => candidate.parentSessionId === parentSessionId).filter((candidate) => description === void 0 || candidate.label === description).sort((left, right) => left.addedAt - right.addedAt || left.sessionId.localeCompare(right.sessionId))[0];
	})();
...
//# sourceMappingURL=http-BrIlNa1k.js.map