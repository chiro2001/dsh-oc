...
function expandRecord(record) {
	if (record.type === "event") return [{ event: record.event }];
	const raw = record.event;
	const kind = raw.type.startsWith("chunkrow/") ? raw.type.slice(9) : raw.type;
	const base = raw.data;
	const elapsed = Array.isArray(base.dt) ? base.dt.reduce((total, gap) => total + (typeof gap === "number" ? gap : 0), 0) : 0;
	return [{ event: {
		type: kind,
		seq: raw.seq,
		...
		switch (event.type) {
			...
			case "assistant/chunk": {
				...
				if (chunk.type === "block-start") blockStarts.set(`${data.turn}:${data.step}:${chunk.index}:${chunk.blockType}`, event.time);
					...
					upsertPartialV1(entries, pending, blocksByStep, pendingCallsByStep, opts, data.turn, data.step, created, lastMessageId || `pending:${opts.sessionId}:user`);
				}
				break;
			}
			case "text-chunks":
		...
		switch (event.type) {
			...
			case "assistant/chunk": {
				...
				if (chunk.type === "block-start") blockStarts.set(`${data.turn}:${data.step}:${chunk.index}:${chunk.blockType}`, event.time);
					...
					const start = blockStarts.get(key) ?? event.time;
					accumulateStreamBlock(blocksByStep, data.turn, data.step, chunk.index, blockType, chunk.text, start);
					lastAssistant = upsertPartialV2(messages, pending, blocksByStep, pendingCallsByStep, opts, data.turn, data.step, assistantCreatedAt(data.turn, data.step, event.time), event.seq, (message, seq) => pushMessage(message, seq));
				}
				break;
			}
			case "text-chunks":
...
//#endregion
//#region node_modules/.pnpm/@deepseek-ai+dsh-util-crypto@0.1.2-rc.1_@deepseek-ai+cordis@4.0.1/node_modules/@deepseek-ai/dsh-util-crypto/lib/index.js
/**
...
//#endregion
//#region node_modules/.pnpm/@deepseek-ai+dsh-brand@0.1.2-rc.1_@deepseek-ai+cordis@4.0.1/node_modules/@deepseek-ai/dsh-brand/lib/index.js
/**
...
//#endregion
//#region node_modules/.pnpm/@deepseek-ai+dsh-util-values@0.1.2-rc.1_@deepseek-ai+cordis@4.0.1/node_modules/@deepseek-ai/dsh-util-values/lib/index.js
/**
...
//#endregion
//#region node_modules/.pnpm/@deepseek-ai+dsh-llm@0.1.2-rc.1_@deepseek-ai+cordis@4.0.1/node_modules/@deepseek-ai/dsh-llm/lib/types/message.js
...
function createBridgeRouter(api, options = {}) {
	...
	function frameSessionId(frame) {
		if (frame.type === "session/event" || frame.type === "session/queue" || frame.type === "session/jobs" || frame.type === "session/projection" || frame.type === "approval/requested" || frame.type === "approval/resolved" || frame.type === "question/requested" || frame.type === "question/resolved") return String(frame.sessionId);
	}
...
//# sourceMappingURL=http-D4YAR6Za.js.map