	...
	*/
	...
	translate(frame) {
		switch (frame.type) {
			...
			case "session/projection": return this.translateProjection(frame.sessionId, frame.key, frame.value);
			...
			case "control/baseline": {
				...
				for (const [sessionId, projection] of Object.entries(frame.value.projections ?? {})) for (const [key, value] of Object.entries(projection.values ?? {})) events.push(...this.translateProjection(sessionId, key, value));
				return events;
			}
			case "stream/error":
	...
	translateProjection(sessionId, key, value) {
		...
		if (key === "subagent") {
			...
			if (mode === "one-shot" || mode === "continuable") this.deps.state.sessionAddressModes.set(sessionId, mode);
			return [];
		}
	...
	translateSessionEvent(sessionId, event, view) {
		...
		switch (event.type) {
			...
			case "assistant/message": {
				...
				const created = state.provisionalMessageCreatedAt.get(stepKey) ?? earliestBlockStart(state.blockStarts, event.data.turn, event.data.step) ?? state.turnStartTime ?? event.time;
				const events = messageEvents(sessionId, this.deps, () => {
			...
			case "turn/end": {
				...
				for (const [stepKey, messageID] of [...state.provisionalMessageIds]) {
					...
					const created = state.blockStarts.get(`${stepKey}:text`) ?? state.blockStarts.get(`${stepKey}:reasoning`) ?? state.provisionalMessageCreatedAt.get(stepKey) ?? state.turnStartTime ?? event.time;
			...
			case "tool/result": {
				...
				const part = resultError === void 0 ? completedToolPart(call, { ...resultInfo }, {
					sessionID: sessionId,
					messageID,
					time: event.time
				}) : errorToolPart(call, {
	...
	translateStreamChunks(sessionId, event, directory, project) {
		...
		if (!block) {
			...
			if (!provisionalId) {
				...
				const provisionalCreated = state.blockStarts.get(blockStartKey) ?? this.deps.state.assistantMessageCreatedAt(sessionId, provisionalId) ?? state.turnStartTime ?? time0;
...
var InteractionState = class InteractionState {
	...
	*/
	...
	setAssistantMessageCreatedAt(sessionId, assistantId, createdAt) {
		if (!this.assistantMessageTimes.has(`${sessionId}\u0000${assistantId}`)) this.assistantMessageTimes.set(`${sessionId}\u0000${assistantId}`, createdAt);
	}
...
function remapV2Messages(ctx, sessionId, messages) {
	...
	for (const message of messages) {
		...
		if (!("content" in message) || !Array.isArray(message.content)) result = {
			...message,
			...surfaceId === void 0 ? {} : { id: surfaceId },
			...sessionAgent !== void 0 && (messageType === "assistant" || messageType === "user") ? { agent: sessionAgent } : {}
		};
		else {
...
function recordSessionSummaries(ctx, items) {
	...
	for (const item of items) {
		...
		const projectedAgent = (item.projections?.values)?.agentPreset;
		...
		if (item.origin === "subagent" && item.parentSessionId !== void 0) {
			...
			const projectionValues = item.projections?.values;
			const projectedMode = projectionValues?.mode ?? (projectionValues?.subagent)?.mode;
			ctx.state.sessionAddressModes.set(id, projectedMode === "one-shot" ? "one-shot" : "continuable");
		}
	}
}
...
//# sourceMappingURL=http-scTid8mI.js.map