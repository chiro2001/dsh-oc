...
function registerSessionV1Routes(register) {
	...
	register("GET", "/session/:id/message", "json", async (req, ctx) => {
		...
		return json(200, remapV1Messages(ctx, id, entries));
	});
	register("GET", "/session/:id/message/:messageID", "json", async (req, ctx) => {
		...
		const found = remapV1Messages(ctx, id, entries).find((entry) => entry.info.id === messageID);
...
//# sourceMappingURL=http-DmZ2FxnL.js.map