	...
	translateSessionEvent(sessionId, event, view) {
		...
		switch (event.type) {
			...
			case "assistant/message": {
				...
				const created = Math.max(provisionalCreated ?? earliestBlockStart(state.blockStarts, event.data.turn, event.data.step) ?? state.turnStartTime ?? event.time, minimumAssistantCreatedAt(state.lastUserMessageCreatedAt));
			...
			case "turn/end": {
				...
				for (const [stepKey, messageID] of [...state.provisionalMessageIds]) {
					...
					const provisionalCreated = state.provisionalMessageCreatedAt.get(stepKey);
					const created = Math.max(provisionalCreated ?? state.blockStarts.get(`${stepKey}:text`) ?? state.blockStarts.get(`${stepKey}:reasoning`) ?? state.turnStartTime ?? event.time, minimumAssistantCreatedAt(state.lastUserMessageCreatedAt));
...
var InteractionState = class InteractionState {
	...
	*/
	...
	setAssistantMessageCreatedAt(sessionId, assistantId, createdAt) {
		...
		if (existing === void 0 || createdAt > existing) this.assistantMessageTimes.set(key, createdAt);
	}
...
function remapV1Messages(ctx, sessionId, entries) {
	...
	for (const entry of entries) {
		...
		if (assistantId !== void 0) {
			...
			const time = info.time;
			if (created !== void 0) {
				const current = typeof time?.created === "number" ? time.created : void 0;
				const normalizedTime = {
					...time ?? {},
					created: current === void 0 ? created : Math.max(current, created)
				};
...
//# sourceMappingURL=http-yg40SgLF.js.map