import { OPENCODE_BIN_ENV } from "../index.js";
...
async function resolveOpenCodeBinary$1(deps = {}) {
	...
	if (override !== void 0) {
		...
		if (exists(override) && await matches(override)) return {
			bin: override,
			source: "env"
		};
	}
...
function prepareOpenCodeTuiState(dshHome, env = process.env) {
	...
	const keybinds = typeof tuiConfig.keybinds === "object" && tuiConfig.keybinds !== null && !Array.isArray(tuiConfig.keybinds) ? tuiConfig.keybinds : {};
	writeFileSync(tuiConfigPath, `${JSON.stringify({
		...tuiConfig,
		keybinds: {
			...keybinds,
...
var OcTuiService = class extends Service {
	...
	async [Service.init]() {
		...
		const { pass: tuiArgs, ignored } = filterSupportedArgs([...this.config.args ?? [], ...this.ctx.cmdlineArgs?.get() ?? []]);
...
//#endregion
export { DSH_OC_TUI_TIMESTAMPS, OPENCODE_KV_FILE, OPENCODE_TUI_FILE, OcTuiConfig, OcTuiService, OcTuiService as default, buildChildEnv, filterSupportedArgs, installSignalForwarding, prepareOpenCodeTuiState, requestExit, resolveAssetUrl, resolveOpenCodeBinary, startOpenCodeTui, tuiTimestampsEnabled };
