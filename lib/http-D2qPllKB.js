	...
	try {
		...
		switch (method) {
			...
			case "session.models":
				...
				return { current: (await api.sessionController.modelCatalog()).default };
			...
			case "session.history": {
				...
				const first = await api.sessionController.follow(followRequest, sig)[Symbol.asyncIterator]().next();
	...
	for (const entry of entries) {
		...
		const mapped = {
			info: {
				...
				...sessionAgent !== void 0 && entry.info.role === "assistant" ? {
					agent: sessionAgent,
					mode: sessionAgent
				} : {},
				...sessionAgent !== void 0 && entry.info.role === "user" ? { agent: sessionAgent } : {}
			},
			parts: entry.parts.map((part) => ({
				...part,
				...surfaceId === void 0 ? {} : {
...
function registerSessionV1Routes(register) {
	...
	register("POST", "/session/:id/message", "json", async (req, ctx) => {
		...
		await broadcastPromptUserMessage(ctx, id, promptUserID, promptText$1(content), Date.now(), bodyModelRef(req.body));
	...
	register("POST", "/session/:id/prompt_async", "json", async (req, ctx) => {
		...
		await broadcastPromptUserMessage(ctx, id, promptUserID, promptText$1(content), Date.now(), bodyModelRef(body));
...
function registerSessionV2Routes(register) {
	...
	register("POST", "/api/session/:sessionID/prompt", "json", async (req, ctx) => {
		...
		await applyAgentFromBody(ctx, id, req.body);
		await broadcastPromptUserMessage(ctx, id, promptUserID, promptText(content), Date.now(), bodyModelRef(req.body));
		if (!await applyModelSelection(ctx, id, req.body)) await reconcileModelSelection(ctx, id);
		await rpc(ctx, "session.prompt", {
			sessionId: sid(id),
			mode: "steer",
...
function recordSessionSummaries(ctx, items) {
	...
	for (const item of items) {
		...
		const agent = item.agentPreset ?? item.agent;
...
function createBridgeRouter(api, options = {}) {
	...
	async function feed(frame) {
		...
		try {
			for (const event of t.translate(frame)) hub.broadcast([event]);
		} catch (error) {
			log(`[bridge/sse] frame translate failed: ${error instanceof Error ? error.message : String(error)}`);
		}
	}
	function feedHostFrame(frame) {
		if (frame.type === "host/agent-error") for (const event of agentErrorEvents(frame.sessionId, frame.message, cwd)) hub.broadcast([event]);
		else if (frame.type === "host/session-added" || frame.type === "host/session-removed") {
			ctx.state.invalidateSession();
			scheduleListRefresh();
			if (frame.type === "host/session-added") for (const event of hostSessionAddedEvents(ctx, frame)) hub.broadcast([event]);
		}
	}
	return {
		ctx,
...
//# sourceMappingURL=http-D2qPllKB.js.map