...
function assistantMessageInfo(message, time, parentID, opts, usage, created) {
	return {
...
function convertMessagesV1(events, opts) {
	...
	for (const event of events) switch (event.type) {
		...
		case "assistant/message": {
			...
			entries.push({
				info: assistantMessageInfo(data.message, event.time, lastMessageId || id, opts, data.usage, earliestBlockStart$1(blockStarts, data.turn, data.step) ?? turnStarts.get(data.turn) ?? event.time),
				parts
			});
...
/** Single-event v1 conversion used by the SSE bridge. */
function assistantMessageFromEvent(event, opts, blockStart, created, parentID) {
	...
	translateSessionEvent(rpcId, sessionId, event) {
		...
		switch (event.type) {
			...
			case "assistant/message": {
				...
				const events = messageEvents(sessionId, this.deps, () => {
					const entry = assistantMessageFromEvent(event, messageOptions(sessionId, this.deps), (index, blockType) => state.blockStarts.get(`${event.data.turn}:${event.data.step}:${index}:${blockType}`), created, state.lastUserMessageId);
					return {
						info: entry.info,
						parts: entry.parts
					};
