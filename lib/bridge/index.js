...
var MuxEventTranslator = class {
	...
	* will reuse. The official TUI renders part updates before the message
	* exists, so no provisional `message.updated` is emitted: the final
	* `assistant/message` supplies the single message card.
	*/
	ensureProvisionalMessage(sessionId, turn, step, time, directory, project) {
		...
		const messageID = this.deps.state.assistantIdForUser(sessionId, state.lastUserMessageId ?? "") ?? provisionalMessageId(sessionId, turn, step);
	...
	*/
	...
	translateSessionEvent(rpcId, sessionId, event, view) {
		...
		switch (event.type) {
			...
			case "user/message": {
				...
				this.deps.log(`[debug] user/message dsh=${dshId} surface=${surfaceId} last=${this.streamState(sessionId).lastUserMessageId}`);
				if (surfaceId !== dshId) {
			...
			case "assistant/message": {
				...
				this.deps.log(`[debug] assistant/message dsh=${dshId} lastUser=${state.lastUserMessageId} bridgeForUser=${bridgeForUser ?? "-"} streamed=${streamed}`);
			...
			case "step/end": {
				const stepKey = `${event.data.turn}:${event.data.step}`;
				const pending = this.pendingAssistantCompletions.get(sessionId);
				if (pending === void 0 || pending.stepKey !== stepKey) return [];
				this.pendingAssistantCompletions.delete(sessionId);
				return [makeEvent(directory, "message.updated", {
					sessionID: sessionId,
					info: {
						...pending.info,
						time: {
	...
	translateStreamChunks(sessionId, event, directory, project) {
		...
		if (!block) {
			...
			if (!provisionalId) {
				provisionalId = this.deps.state.assistantIdForUser(sessionId, state.lastUserMessageId ?? "") ?? provisionalMessageId(sessionId, event.data.turn, event.data.step);
...
function remapV1Messages(ctx, sessionId, entries) {
	return entries.map((entry) => {
		...
		if (surfaceId === void 0) return entry;
		return {
			info: {
				...entry.info,
				id: surfaceId
...
function remapV2Messages(ctx, sessionId, messages) {
	return messages.map((message) => {
		...
		const surfaceId = promptId ?? assistantId;
		if (surfaceId === void 0) return message;
