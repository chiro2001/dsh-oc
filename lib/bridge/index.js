...
/** Single-event v1 conversion used by the SSE bridge. */
function assistantMessageFromEvent(event, opts, blockStart, created) {
	...
	translateSessionEvent(rpcId, sessionId, event) {
		...
		switch (event.type) {
			...
			case "assistant/message": {
				...
				const events = messageEvents(sessionId, this.deps, () => {
					const entry = assistantMessageFromEvent(event, messageOptions(sessionId, this.deps), (index, blockType) => state.blockStarts.get(`${event.data.turn}:${event.data.step}:${index}:${blockType}`), created);
					return {
						info: entry.info,
						parts: entry.parts
					};
