...
function assistantPartsFromMessage(message, time, opts, blockStart, partIdFor) {
	const parts = [];
...
/** Single-event v1 conversion used by the SSE bridge. */
function assistantMessageFromEvent(event, opts, blockStart, created, parentID, finish, partIdFor) {
	...
	translateSessionEvent(rpcId, sessionId, event, view) {
		...
		switch (event.type) {
			...
			case "assistant/message": {
				...
				const events = messageEvents(sessionId, this.deps, () => {
					const entry = assistantMessageFromEvent(event, messageOptions(sessionId, this.deps), (index, blockType) => state.blockStarts.get(`${event.data.turn}:${event.data.step}:${index}:${blockType}`), created, state.lastUserMessageId, state.finishReasons.get(stepKey) ?? "stop", (index, blockType) => {
...
function seedDerivedHistoryPage(ctx, sessionId, value, options) {
	...
	if (options.maxMessages === void 0) {
		...
		if (ctx.state.getHistoryCache(pageKey, HISTORY_CACHE_MS) === void 0) ctx.state.setHistoryCache(pageKey, {
		...
		if (ctx.state.getHistoryCache(tailKey, HISTORY_CACHE_MS) === void 0) ctx.state.setHistoryCache(tailKey, value);
	}
}
...
function createBridgeRouter(api, options = {}) {
	...
	register("GET", "/session", "json", async (_req, ctx) => {
		const items = filterSessionsByDirectory(await cachedSessionList(ctx), _req.query.get("directory") ?? void 0, cwd);
	...
	register("GET", "/session/status", "json", async (_req, ctx) => {
		const list = await cachedSessionList(ctx);
		const status = {};
		for (const item of filterSessionsByDirectory(list, _req.query.get("directory") ?? void 0, cwd)) status[String(item.sessionId)] = item.running ? { type: "busy" } : { type: "idle" };
	...
	register("POST", "/session/:id/prompt_async", "json", async (req, ctx) => {
		...
		const agent = typeof body.agent === "string" && body.agent.length > 0 ? body.agent : void 0;
		if (agent !== void 0 && agent !== DEFAULT_AGENT_NAME) try {
	...
	register("GET", "/api/session", "json", async (req, ctx) => {
		...
		const filtered = filterSessionsByDirectory(all, req.query.get("directory") ?? void 0, cwd);
		const limitRaw = req.query.get("limit");
	...
	return {
		...
		prefetchSessionList() {
			(async () => {
				try {
					const items = await cachedSessionList(ctx);
					await Promise.allSettled(items.slice(0, RECENT_HISTORY_PREFETCH).map((item) => cachedSessionHistory(ctx, String(item.sessionId), { maxMessages: 100 })));
				} catch (error) {
