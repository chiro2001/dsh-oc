...
async function forkTitleForSource(ctx, sourceId) {
	...
	const existingForks = list.items.filter((item) => String(item.sessionId) !== sourceId && String(item.parentSessionId) === sourceId && item.origin !== "subagent");
	return `${(source === void 0 ? void 0 : sessionTitleFrom(source)) || "Session"} (fork #${existingForks.length + 1})`;
}
