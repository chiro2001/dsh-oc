...
function registerRoutes(register) {
	...
	register("POST", "/session/:id/prompt_async", "json", async (req, ctx) => {
		...
		if (agent !== void 0 && agent !== "build") try {
...
async function warmListTitles(ctx, items) {
	...
	const syncCount = missing.length <= 40 ? missing.length : Math.min(12, missing.length);
	await warmTitles(ctx, missing.slice(0, syncCount));
	if (missing.length > syncCount) warmTitles(ctx, missing.slice(syncCount, 120));
}
