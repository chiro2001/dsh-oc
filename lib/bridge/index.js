...
function assistantPartsFromMessage(message, time, opts) {
	const parts = [];
	const calls = /* @__PURE__ */ new Map();
	const messageID = String(message.id);
	message.content.forEach((block, index) => {
		if (block.type === "text") parts.push(textPart(`${messageID}:${index}`, messageID, block.text, time, opts));
...
function convertMessagesV1(events, opts) {
	...
	for (const event of events) switch (event.type) {
		...
		case "assistant/message": {
			...
			const { parts, calls: messageCalls } = assistantPartsFromMessage(data.message, event.time, opts);
...
/** Single-event v1 conversion used by the SSE bridge. */
function assistantMessageFromEvent(event, opts) {
...
function convertToProviderCatalog(groups) {
	return {
		all: groups.map((group) => {
			const providerId = externalProviderId(group.id);
			const models = {};
			for (const model of group.models) models[model.id] = {
				id: model.id,
