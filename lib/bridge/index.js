...
function convertMessagesV1(events, opts, views) {
	...
	for (let eventIndex = 0; eventIndex < events.length; eventIndex++) {
		...
		switch (event.type) {
			...
			case "tool/result": applyToolResultV1(entries, calls, event, opts, view);
		}
	}
	return entries;
}
...
/** Single-event v1 conversion used by the SSE bridge. */
function assistantMessageFromEvent(event, opts, blockStart, created, parentID, finish) {
...
function streamPart(blockType, sessionId, messageId, partId, text, start) {
	return {
		id: partId,
		sessionID: sessionId,
		messageID: messageId,
	...
	translateSessionEvent(rpcId, sessionId, event, view) {
		...
		switch (event.type) {
			case "user/message": {
				...
				if (isCompactCheckpoint(event)) events.push(makeEvent(directory, "session.next.compaction.ended", {
			...
			case "assistant/message": {
				...
				const events = messageEvents(sessionId, this.deps, () => {
					const entry = assistantMessageFromEvent(event, messageOptions(sessionId, this.deps), (index, blockType) => state.blockStarts.get(`${event.data.turn}:${event.data.step}:${index}:${blockType}`), created, state.lastUserMessageId, state.finishReasons.get(stepKey) ?? "stop");
					return {
						info: entry.info,
						parts: entry.parts
					};
				...
				if (provisionalId) {
					events.unshift(makeEvent(directory, "message.removed", {
						sessionID: sessionId,
			...
			case "tool/call": {
				...
				return [makeEvent(directory, "message.part.updated", {
					...
					part: pendingToolPart(call, {
						sessionID: sessionId,
						messageID: this.currentAssistant.get(sessionId) ?? this.streamState(sessionId).provisionalMessageIds.get(`${data.turn}:${data.step}`) ?? `assistant:${data.turn}:${data.step}`,
						time: event.time
					}),
					time: event.time
				}, project)];
...
function fileChangeEvents(sessionID, messageID, changes, project, directory, time) {
	...
	const patch = changes.map((change) => change.patch).filter((value) => value !== void 0).join("\n");
...
function recordSessionSummaries(ctx, items) {
	...
	for (const item of items) {
		...
		if (item.parentSessionId !== void 0) ctx.state.sessionParents.set(id, String(item.parentSessionId));
	}
}
...
async function forkSession(req, ctx, v2) {
	...
	const atSeq = messageId === void 0 ? void 0 : await atSeqForMessage(ctx, id, messageId);
...
*/
async function compactSession(ctx, sessionId) {
	const agent = ctx.api.agents?.get(sessionId);
	if (agent === void 0) throw conflict("session is not attached; cannot compact", { sessionId });
	if (!ctx.api.commands) throw internalError("dsh command registry is unavailable; cannot compact", { sessionId });
	let execution;
	try {
		execution = await ctx.api.commands.execute(agent, "/compact", new AbortController().signal);
	} catch (error) {
		throw internalError(`compact command failed: ${error instanceof Error ? error.message : String(error)}`, { sessionId });
	}
	if (execution === void 0) throw badRequest("unknown command /compact", {
		code: "unknown-command",
		sessionId
	});
	if (execution.result.kind === "error") throw badRequest(execution.result.text ?? "compact command failed", {
		code: "command-error",
		sessionId
	});
	if (execution.result.text) ctx.log(`[bridge] /compact: ${execution.result.text}`);
}
async function createSession(req, ctx, v2) {
	...
	const agentPreset = agentName === void 0 ? void 0 : await presetIdForAgent(ctx, agentName);
	let id;
	if (parentID) {
...
function createBridgeRouter(api, options = {}) {
	...
	register("GET", "/session", "json", async (_req, ctx) => {
		const list = await rpc(ctx, "session.list", {});
		recordSessionSummaries(ctx, list.items);
		return json(200, list.items.map((item) => convertSessionSummary(item, { cwd: state.sessionDirectories.get(String(item.sessionId)) ?? cwd })));
	});
	...
	register("POST", "/session/:id/command", "json", async (req, ctx) => {
		...
		if (argument === "") {
			...
			const text = roster.length === 0 ? "No switchable dsh agent presets" : roster.map((preset) => `${preset.id}${preset.isDefault ? " (default)" : ""}`).join("\n");
	...
	register("GET", "/session/:id/diff", "json", async (req, ctx) => {
		const id = req.params.id;
		return json(200, producedFilesV1(historyFileDiffs(await rpc(ctx, "session.history", { sessionId: sid(id) }))));
	});
	...
	register("GET", "/api/session", "json", async (_req, ctx) => {
		...
		recordSessionSummaries(ctx, list.items);
		return json(200, {
			data: list.items.map((item) => convertSessionSummaryV2(item, { cwd: state.sessionDirectories.get(String(item.sessionId)) ?? cwd })),
			cursor: {}
		});
	});
	...
	register("GET", "/api/session/:sessionID/message", "json", async (req, ctx) => {
		...
		const entries = history.events;
		return json(200, {
			data: convertMessagesV2(entries.map((entry) => entry.event), {
				sessionId: id,
				cwd,
				defaultModel,
	...
	register("GET", "/api/session/:sessionID/diff", "json", async (req, ctx) => {
		const id = req.params.sessionID;
		return json(200, historyFileDiffs(await rpc(ctx, "session.history", { sessionId: sid(id) })));
	});
	...
	function startSse(_req, res) {
		...
		(async () => {
			try {
				...
				for await (const frame of stream) for (const event of translator.translate(frame)) hub.send(client, event);
			} catch (error) {
...
var OcBridgeService = class extends Service {
	...
	async *[Service.init]() {
		...
		const handle = await startBridgeServer(createBridgeRouter({
			...apiProxy,
