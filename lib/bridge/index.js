...
function remapV2Messages(ctx, sessionId, messages) {
	return messages.map((message) => {
		...
		const surfaceId = promptId ?? assistantId;
		const sessionAgent = ctx.state.sessionAgentFor(sessionId);
		if (surfaceId === void 0 && sessionAgent === void 0) return message;
...
function registerSessionV2Routes(register) {
	...
	register("GET", "/api/session/:sessionID/history", "json", async (req, ctx) => {
		...
		const filtered = after === void 0 ? withSeq : withSeq.filter((entry) => entry.seq > after);
		const page = filtered.slice(0, limit ?? filtered.length);
		const next = page.length === 0 ? null : page.reduce((max, entry) => Math.max(max, entry.seq), -1);
		return json(200, {
