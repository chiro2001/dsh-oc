...
var InteractionState = class InteractionState {
	...
	invalidateHistory(sessionId) {
		...
		for (const key of [...this.historyCache.keys()]) if (key === sessionId || key.startsWith(`${sessionId}:`)) this.historyCache.delete(key);
	}
...
async function skillList(ctx, directory) {
	...
	try {
		return (await rpc(ctx, "skill.list", { sessionId: sid(String(session.sessionId)) })).skills.map((skill) => ({
			name: skill.name,
...
async function skillListForSession(ctx, sessionId) {
	try {
		return (await rpc(ctx, "skill.list", { sessionId: sid(sessionId) })).skills.map((skill) => ({
			name: skill.name,
			description: skill.description
		}));
	} catch (error) {
...
function createBridgeRouter(api, options = {}) {
	...
	register("GET", "/session", "json", async (_req, ctx) => {
		const items = filterSessionsByDirectory(await cachedSessionList(ctx), _req.query.get("directory") ?? void 0);
	...
	register("GET", "/session/status", "json", async (_req, ctx) => {
		const list = await cachedSessionList(ctx);
		const status = {};
		for (const item of filterSessionsByDirectory(list, _req.query.get("directory") ?? void 0)) status[String(item.sessionId)] = item.running ? { type: "busy" } : { type: "idle" };
