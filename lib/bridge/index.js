...
function createBridgeRouter(api, options = {}) {
	...
	register("GET", "/api/session", "json", async (req, ctx) => {
		...
		const items = limit === void 0 ? filtered : filtered.slice(0, limit);
		const ordered = req.query.get("order") === "asc" ? [...items].reverse() : items;
		recordSessionSummaries(ctx, ordered);
		return json(200, {
			data: ordered.map((item) => convertSessionSummaryV2(item, { cwd: state.sessionDirectories.get(String(item.sessionId)) ?? cwd })),
			cursor: {}
		});
	});
			...
			try {
				...
				for await (const frame of stream) {
					...
					if (frame.payload.type === "session/event") {
						...
						if (sessionEvent.type === "session" || sessionEvent.type === "session/created" || sessionEvent.type === "session/title") try {
							const list = await rpc(ctx, "session.list", {});
							recordSessionSummaries(ctx, list.items);
						} catch (error) {
