...
async function warmListTitles(ctx, items) {
	...
	const syncCount = missing.length <= LIST_TITLE_WARM_ALL_MAX ? missing.length : Math.min(LIST_TITLE_WARM_LIMIT, missing.length);
	await warmTitles(ctx, missing.slice(0, syncCount));
	if (missing.length > syncCount) warmTitles(ctx, missing.slice(syncCount, LIST_TITLE_WARM_BACKGROUND_MAX));
}
