import { n as ocHelp } from "../help-DBhVY-No.js";
...
function agentErrorEvent(sessionId, message, cwd) {
...
function filterSessionsByDirectory(items, directory) {
...
async function completeGoalCommand(ctx, sessionId) {
	...
	try {
		const current = goalFromHistory(await rpc(ctx, "session.history", { sessionId: sid(sessionId) }));
...
async function forkTitleForSource(ctx, sourceId) {
	...
	return `${base} (fork #${list.items.filter((item) => String(item.sessionId) !== sourceId && String(item.parentSessionId) === sourceId && item.origin !== "subagent").length + 1})`;
}
...
function createBridgeRouter(api, options = {}) {
	...
	register("GET", "/command", "json", async () => json(200, [
		PRESET_COMMAND_V1,
		GOAL_COMMAND_V1,
		HELP_COMMAND_V1
	]));
	...
	register("GET", "/session", "json", async (_req, ctx) => {
		const items = filterSessionsByDirectory((await rpc(ctx, "session.list", {})).items, _req.query.get("directory") ?? void 0);
		recordSessionSummaries(ctx, items);
		return json(200, items.map((item) => convertSessionSummary(item, { cwd: state.sessionDirectories.get(String(item.sessionId)) ?? cwd })));
	});
	register("GET", "/session/status", "json", async (_req, ctx) => {
		const list = await rpc(ctx, "session.list", {});
		const status = {};
		for (const item of filterSessionsByDirectory(list.items, _req.query.get("directory") ?? void 0)) status[String(item.sessionId)] = item.running ? { type: "busy" } : { type: "idle" };
	...
	register("GET", "/api/session", "json", async (req, ctx) => {
		...
		if (search !== null && search.length > 0) {
			...
			all = (await rpc(ctx, "session.list", {})).items.filter((item) => ids.has(String(item.sessionId)));
		} else all = (await rpc(ctx, "session.list", {})).items;
		const filtered = filterSessionsByDirectory(all, req.query.get("directory") ?? void 0);
		const limitRaw = req.query.get("limit");
		const limit = limitRaw ? Math.max(1, Math.min(Number(limitRaw) || 100, 500)) : 100;
		...
		return json(200, {
			data: page.map((item) => convertSessionSummaryV2(item, { cwd: state.sessionDirectories.get(String(item.sessionId)) ?? cwd })),
			cursor: {
	...
	register("GET", "/api/session/:sessionID/message", "json", async (req, ctx) => {
		...
		const cursorRaw = req.query.get("cursor");
		const beforeSeq = cursorRaw === null ? void 0 : decodeMessageCursor(cursorRaw);
		const history = await rpc(ctx, "session.history", {
			sessionId: sid(id),
			...limit === void 0 ? {} : { maxMessages: limit },
			...
			try {
				...
				(async () => {
					for await (const frame of hostStream) {
						...
						if (payload.type === "host/session-added" || payload.type === "host/session-removed") scheduleListRefresh();
					}
				})().catch((error) => {
					if (!controller.signal.aborted) log(`[bridge/sse] host stream ended: ${error instanceof Error ? error.message : String(error)}`);
				});
				for await (const frame of stream) {
					...
					if (frame.payload.type === "session/event") {
						...
						if (sessionEvent.type === "session" || sessionEvent.type === "session/created" || sessionEvent.type === "session/title") scheduleListRefresh();
					}
