...
async function runGoalCommand(ctx, sessionId, argument) {
	...
	return runRegistryCommand(ctx, sessionId, trimmed === "" ? "/goal" : `/goal ${trimmed}`, "/goal");
}
...
function createBridgeRouter(api, options = {}) {
	...
	register("POST", "/session/:id/message", "json", async (req, ctx) => {
		...
		if (slash !== void 0) {
			const outcome = slash.name === "preset" ? await runPresetCommand(ctx, id, slash.argument) : await runGoalCommand(ctx, id, slash.argument);
