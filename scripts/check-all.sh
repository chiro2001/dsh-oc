...
if [[ "$E2E" == "1" ]]; then
  ...
  if [[ "${CI_E2E_SUBSET:-}" == "api" ]]; then
    ...
    E2E_SCRIPTS="$STABLE_E2E e2e-tui-timestamps.sh e2e-tui-mini.sh e2e-tui-skill.sh e2e-tui-continue.sh e2e-tui-dir-filter.sh e2e-tui-abort.sh e2e-tui-permission.sh"
  fi
