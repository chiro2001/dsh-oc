 ...
 */
function expandRecord(record: SessionHistoryRecord): HistoryEntry[] {
  ...
  try {
    ...
    switch (method) {
      ...
      case 'session.history': {
        ...
        const address: SessionAddress = { kind: 'session', sessionId: sessionId as never }
        const followRequest: SessionFollowRequest = {
          address,
          ...(maxMessages === undefined ? {} : { maxMessages }),
        }
        // The first follow frame is the opening snapshot: records, projections,
        // hasMore and the cursor. beforeSeq pagination then reads backwards via
        // page against that cursor.
        const it = api.sessionController.follow(followRequest, sig)[Symbol.asyncIterator]()
        ...
        const projections = snapshot.projections as SessionProjectionsBlock | undefined
        return {
