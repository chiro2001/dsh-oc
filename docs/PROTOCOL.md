...
| `GET /api/session/active` | MAP | 当前活动会话，仅当实际 `running` 时返回 `{ data: { [sessionId]: { type: "running" } } }`，否则 `{}` |
| `POST /api/session/{id}/wait` | MAP | 等待 agent 循环空闲：≤30s 轮询 `session.list`，空闲 204、超时 503、会话不存在 404 |
