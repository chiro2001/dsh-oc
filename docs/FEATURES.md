...
> 运行 `pnpm run features:update` 重新生成。生成时 HEAD：`b38c26c`（2026-08-15）。
...
| `tests/bridge-router.spec.ts` | 48 | b38c26c feat(bridge): filter session lists by directory query |
...
| `src/bridge/router.ts` | b38c26c feat(bridge): filter session lists by directory query |
