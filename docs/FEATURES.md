...
> 运行 `pnpm run features:update` 重新生成。生成时 HEAD：`0d72edc`（2026-08-16）。
...
| `tests/bridge-router.spec.ts` | 79 | 0d72edc feat(bridge): add subagent children listing endpoint |
...
| `scripts/e2e-api.sh` | 0d72edc feat(bridge): add subagent children listing endpoint |
