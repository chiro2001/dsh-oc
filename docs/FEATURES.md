...
> 运行 `pnpm run features:update` 重新生成。生成时 HEAD：`c76acdb`（2026-08-16）。
...
| `tests/bridge-router.spec.ts` | 79 | 3b56c7c feat(bridge): enable background subagents capability and background endpoint |
...
| `src/bridge/routes/session-v2.ts` | 2e52727 feat(bridge): add v2 interrupt route and dedupe replayed stream chunks |
...
| `scripts/e2e-api.sh` | c76acdb test(e2e): assert background subagents capability and no-op endpoint |
