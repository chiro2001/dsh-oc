...
> 运行 `pnpm run features:update` 重新生成。生成时 HEAD：`0e90a74`（2026-08-16）。
...
| `tests/bridge-router.spec.ts` | 82 | 0e90a74 feat(bridge): add bounded session wait endpoint |
...
| `src/bridge/routes/session-v2.ts` | 0e90a74 feat(bridge): add bounded session wait endpoint |
...
| `scripts/e2e-api.sh` | 0e90a74 feat(bridge): add bounded session wait endpoint |
