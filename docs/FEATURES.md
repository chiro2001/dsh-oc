...
> 运行 `pnpm run features:update` 重新生成。生成时 HEAD：`267963c`（2026-08-16）。
...
| `tests/bridge-router.spec.ts` | 82 | 1ac16c9 feat(bridge): report the active session through v2 endpoint |
...
| `src/bridge/routes/session-v2.ts` | 1ac16c9 feat(bridge): report the active session through v2 endpoint |
...
| `scripts/e2e-api.sh` | 267963c test(e2e): cover health and active-session endpoints in api matrix |
