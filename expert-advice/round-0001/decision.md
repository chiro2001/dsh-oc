...
### 实验 1 已知错序部分：已完成裁决（2026-08-16 深夜）

- 修复：v1/v2 历史按桥接 id 合并同回合工具调用与后续文本为单条消息，
  与实时 SSE 一致；同一桥接 id 不再在历史中重复出现。
- 证据：规范化 v1/v2 历史 exactly-once、父子正确；
  `--session` 重新进入会话后转录顺序完全正确。
- 裁决：即时视图错序为上游 TUI 渲染行为（对“排队卡之后到达的同消息
  part”的合并）；停止桥接层继续试错，作为已知限制文档化。
- 未完成部分：live/重连/重启/恢复一致性的完整差分回放仍列为后续；
  10k 事件单会话 history 性能基准仍待跑。

### 实验 1 恢复一致性部分：已完成初版（2026-08-16）

- 修复：同一回合的工具调用与后续文本改用**独立消息 id**（不再依赖进程内
  bridge id 映射合并），使实时视图与 `--session` 新进程重连后的历史结构
  一致（此前实时 4 条、重连 5 条，结构不一致）。
- 新增 `scripts/e2e-recovery-consistency.sh`：实时快照 vs 新进程重连快照，
  断言消息数、角色、父链与部件完全一致（33s PASSED），已纳入稳定套件。
- 10k 事件 history 性能与分页基线已完成（见 perf 文档）。
- 剩余：SSE 断线重连、bridge 崩溃重启、`--continue` 变体与 10+ 脱敏真实
  会话回放仍为后续。
